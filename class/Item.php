...
class Item extends Database implements iItem{
	...
	public function insert_item($iN, $aT, $sN, $mN, $b, $a, $pD, $eID, $cID, $coID)
	{
		$sql = "INSERT INTO tbl_item(item_name, asset_tag, item_serno, item_modno, item_brand, item_amount, item_purdate, emp_id, cat_id, con_id)
				VALUES(?,?,?,?,?,?,?,?,?,?);
		";
		$result = $this->insertRow($sql, [$iN, $aT, $sN, $mN, $b, $a, $pD, $eID, $cID, 1]);
		return $result;
	}

	public function update_item($iN, $aT, $sN, $mN, $b, $a, $pD, $eID, $cID, $coID, $iID)
	{	
		$sql="UPDATE tbl_item
			  SET 
			  item_name = ?, 
			  ...
			  WHERE item_id = ?
		";
		$result = $this->updateRow($sql, [$iN, $aT, $sN, $mN, $b, $a, $pD, $eID, $cID, $coID, $iID]);
		return $result;
	}
